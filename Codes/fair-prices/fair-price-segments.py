...
try:
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("DELETE FROM fair_price_segments;")
        
        cursor.execute("ALTER TABLE fair_price_segments AUTO_INCREMENT = 1;")
        ...
        # Select the desired columns from weighted_segments
        cursor.execute("""
            SELECT `key`,
            ...
            FROM weighted_segments;
        """)
        results = cursor.fetchall()

        # Insert the selected data into fair_price_segments
        for row in results:
            ...
            cursor.execute("""
                INSERT INTO fair_price_segments (`key`, marketCap, priceEarnings, forwardPE, trailingEps, forwardEps, bookValue, priceToBook, 
            ...
            cursor.execute("""
                SELECT economicSector FROM weighted_segments WHERE `key` = %s;
            """, (row['key'],))
            ...
            if economicSector_row:
                ...
                if es_values:
                    # Update fair_price_segments with economic sector fair prices
                    cursor.execute("""
                        UPDATE fair_price_segments
                        SET 
                            marketCap_es = %s,
            ...
            cursor.execute("""
                SELECT subsector FROM weighted_segments WHERE `key` = %s;
            """, (row['key'],))
            ...
            if subsector_row:
                ...
                if ss_values:
                    # Update fair_price_segments with subsector fair prices
                    cursor.execute("""
                        UPDATE fair_price_segments
                        SET 
                            marketCap_ss = %s,
        ...
        connection.commit()
        print("Data successfully processed and inserted/updated into 'fair_price_segments'.")
