...
try:
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("DELETE FROM weighted_subsectors;")
        cursor.execute("ALTER TABLE weighted_subsectors AUTO_INCREMENT = 1;")
        ...
        insert_columns = ["`key`"] + [f"weighted_mean_{col}" for col in weighted_columns] + [f"sum_{col}" for col in sum_columns] + ['weighted_mean_regularMarketDayRange', 'weighted_mean_fiftyTwoWeekRange', 'economicSector', 'regularMarketTime']
        ...
        insert_query = f"""
        INSERT INTO weighted_subsectors ({", ".join(insert_columns)})
        ...
        for row in results:
            ...
            data_tuple = (
                ...
                [weighted_mean_regularMarketDayRange, weighted_mean_fiftyTwoWeekRange, row['economicSector'], most_common_time]
            )
                    
            cursor.execute(insert_query, data_tuple)
            
        connection.commit()
        print("Data successfully processed and inserted/updated into 'weighted_subsectors'.")
